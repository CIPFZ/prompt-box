    ...
    <>
      <header className="h-14 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800 flex items-center px-4 gap-3 flex-shrink-0 relative z-30">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            onClick={toggleLang}
            className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 p-2 hover:bg-gray-100 dark:hover:bg-gray-800 rounded-lg transition-colors text-xs font-bold uppercase min-w-[28px]"
          ...
          >
            {isGallery ? tt("newRecord") : tt("backGallery")}
          </Link>
        </div>
      </header>
        ...
        >
          ...
          >
            ...
            <div className="p-5 space-y-4 max-h-[50vh] overflow-y-auto">
              <div>
                <div className="flex items-center justify-between mb-2.5">
                  <span className="text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider">{tt("tags")}</span>
                  {searchTags.length > 0 && (
                    <span className="text-[10px] text-gray-400">{searchTags.length} {tt("selected")}</span>
                  )}
                </div>
                {searchTags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mb-2.5">
                    {searchTags.map((tag) => (
                      <span key={tag} className="flex items-center gap-1 px-2.5 py-1 bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 text-xs rounded-full font-medium">
                        {tag}
                ...
                <div className="flex flex-wrap gap-1.5">
                    ...
                    return (
                      <button
                        ...
                        className={`px-2.5 py-1 text-xs rounded-full font-medium transition-colors ${
                          active ? "bg-blue-600 text-white ring-2 ring-blue-600" : "bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 hover:bg-gray-200 dark:hover:bg-gray-700"
                        }`}
                      >
                        {tag}
                      </button>
                  ...
                  {allTags.length === 0 && (
                    <span className="text-xs text-gray-400 py-2">{tt("noTags")}</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="px-5 py-3 border-t border-gray-100 dark:border-gray-800 flex items-center justify-between text-[10px] text-gray-400 dark:text-gray-500">
              <span>{tt("escClose")}</span>
              {totalCount > 0 && <span>{totalCount} {tt("totalRecords")}</span>}
            </div>
          </div>
        </div>
      )}
