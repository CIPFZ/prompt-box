...
export default function SettingsPage() {
  const { apiConfigs, testResult, testing, saveApiConfig, deleteApiConfig, testConnection, clearTestResult } = useSettingsStore();
  ...
  const [apiProxyEnabled, setApiProxyEnabled] = useState(false);
  ...
  function startEdit(config: (typeof apiConfigs)[number]) {
    ...
    setApiProxyEnabled(config.proxy_enabled);
    setApiProxyUrl(config.proxy_url || "");
    setShowForm(true);
    clearTestResult();
  }
  ...
  async function handleSaveApi() {
    await saveApiConfig({
      ...
      is_active: true,
      proxy_enabled: apiProxyEnabled,
      proxy_url: apiProxyUrl || null,
    });
    resetApiForm();
  }
    ...
    <div className="h-screen flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-3xl mx-auto p-8">
          <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6">{tt("apiConfig")}</h1>
            ...
            <div key={cfg.id} className="flex items-center gap-4 p-4 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="font-medium text-gray-800 dark:text-gray-200 text-sm">
                  {cfg.name}
                  {cfg.proxy_enabled && (
                    <span className="ml-2 inline-flex items-center gap-1 text-[10px] bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 px-1.5 py-0.5 rounded-full">代理</span>
                  )}
                </div>
          ...
          {showForm && (
            <div className="mt-4 p-6 bg-white dark:bg-gray-900 rounded-xl border border-gray-200 dark:border-gray-800 space-y-4">
              ...
              <div>
                ...
                <textarea value={apiExtra} onChange={(e) => setApiExtra(e.target.value)} rows={2} className="w-full p-2 border border-gray-200 dark:border-gray-700 rounded-lg text-sm font-mono outline-none focus:ring-2 focus:ring-blue-500 resize-none bg-white dark:bg-gray-800 dark:text-gray-200" />
              </div>
              ...
              <div className="border-t border-gray-100 dark:border-gray-800 pt-4">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    <h4 className="text-sm font-medium text-gray-800 dark:text-gray-200">{tt("proxyEnabled")}</h4>
                    <p className="text-xs text-gray-400">{tt("proxyDesc")}</p>
                  </div>
                  <button
                    onClick={() => setApiProxyEnabled(!apiProxyEnabled)}
                    className={`relative w-11 h-6 rounded-full transition-colors ${apiProxyEnabled ? "bg-blue-600" : "bg-gray-300 dark:bg-gray-700"}`}
                  >
                    <div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${apiProxyEnabled ? "translate-x-[22px]" : "translate-x-0.5"}`} />
                  </button>
                </div>
                {apiProxyEnabled && (
                  <input
